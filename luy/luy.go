...
func (luy *luy) MonthAbbreviated(month time.Month) string {
	if len(luy.monthsAbbreviated) == 0 {
		return ""
	}
...
func (luy *luy) FmtCurrency(num float64, v uint64, currency currency.Type) string {
	...
	for j := len(luy.currencyPositivePrefix) - 1; j >= 0; j-- {
		b = append(b, luy.currencyPositivePrefix[j])
	}
...
func (luy *luy) FmtAccounting(num float64, v uint64, currency currency.Type) string {
	...
	if num < 0 {
		...
		for j := len(luy.currencyNegativePrefix) - 1; j >= 0; j-- {
			b = append(b, luy.currencyNegativePrefix[j])
		}
