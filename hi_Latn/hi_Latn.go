...
func (hi *hi_Latn) FmtCurrency(num float64, v uint64, currency currency.Type) string {
	...
	for j := len(hi.currencyPositivePrefix) - 1; j >= 0; j-- {
		b = append(b, hi.currencyPositivePrefix[j])
	}
...
func (hi *hi_Latn) FmtAccounting(num float64, v uint64, currency currency.Type) string {
	...
	if num < 0 {
		...
		for j := len(hi.currencyNegativePrefix) - 1; j >= 0; j-- {
			b = append(b, hi.currencyNegativePrefix[j])
		}
