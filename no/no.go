...
func (no *no) FmtCurrency(num float64, v uint64, currency currency.Type) string {
	...
	for j := len(no.currencyPositivePrefix) - 1; j >= 0; j-- {
		b = append(b, no.currencyPositivePrefix[j])
	}
