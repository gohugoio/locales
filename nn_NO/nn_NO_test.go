...
func TestFmtTimeFull(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			expected: "09:05:01 normaltid for den nordamerikanske austkysten",
		},
		{
...
// TestDaysWide_Nynorsk verifies nn_NO has Nynorsk day names, not Bokmål.
func TestDaysWide_Nynorsk(t *testing.T) {
	trans := New()

	expected := []string{"søndag", "måndag", "tysdag", "onsdag", "torsdag", "fredag", "laurdag"}
	days := trans.WeekdaysWide()

	for i, day := range days {
		if day != expected[i] {
			t.Errorf("WeekdaysWide()[%d]: Expected '%s' Got '%s'", i, expected[i], day)
		}
		s := trans.WeekdayWide(time.Weekday(i))
		if s != expected[i] {
			t.Errorf("WeekdayWide(%d): Expected '%s' Got '%s'", i, expected[i], s)
		}
	}
}

// TestDaysAbbreviated_Nynorsk verifies nn_NO has Nynorsk abbreviated days.
func TestDaysAbbreviated_Nynorsk(t *testing.T) {
	trans := New()

	expected := []string{"sø.", "må.", "ty.", "on.", "to.", "fr.", "la."}
	days := trans.WeekdaysAbbreviated()

	for i, day := range days {
		if day != expected[i] {
			t.Errorf("WeekdaysAbbreviated()[%d]: Expected '%s' Got '%s'", i, expected[i], day)
		}
	}
}
...
func TestMonthsWide(t *testing.T) {
	...
	expected := []string{"januar", "februar", "mars", "april", "mai", "juni", "juli", "august", "september", "oktober", "november", "desember"}
	months := trans.MonthsWide()

	for i, month := range months {
		if month != expected[i] {
			t.Errorf("MonthsWide()[%d]: Expected '%s' Got '%s'", i, expected[i], month)
		}
	}
}
