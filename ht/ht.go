...
// New returns a new instance of translator for the 'ht' locale
...
func (ht *ht) FmtNumber(num float64, v uint64) string {
	...
	for i := len(s) - 1; i >= 0; i-- {
		...
		if inWhole {
			if count == 3 {
				for j := len(ht.group) - 1; j >= 0; j-- {
					b = append(b, ht.group[j])
				}
				count = 1
			} else {
				count++
...
// FmtPercent returns 'num' with digits/precision of 'v' for 'ht' and handles both Whole and Real numbers based on 'v'
// NOTE: 'num' passed into FmtPercent is assumed to be in percent already
func (ht *ht) FmtPercent(num float64, v uint64) string {
	s := strconv.FormatFloat(math.Abs(num), 'f', int(v), 64)
	l := len(s) + 5
	b := make([]byte, 0, l)
	...
	b = append(b, ht.percentSuffix...)

	b = append(b, ht.percent...)

	return string(b)
}

// FmtCurrency returns the currency representation of 'num' with digits/precision of 'v' for 'ht'
func (ht *ht) FmtCurrency(num float64, v uint64, currency currency.Type) string {

	s := strconv.FormatFloat(math.Abs(num), 'f', int(v), 64)
	symbol := ht.currencies[currency]
	...
	b = append(b, ht.currencyPositiveSuffix...)
...
// FmtAccounting returns the currency representation of 'num' with digits/precision of 'v' for 'ht'
// in accounting notation.
func (ht *ht) FmtAccounting(num float64, v uint64, currency currency.Type) string {
	...
	if num < 0 {
		...
		b = append(b, ht.currencyNegativePrefix[0])
	...
	if num < 0 {
		b = append(b, ht.currencyNegativeSuffix...)
		b = append(b, symbol...)
	} else {
...
// FmtDateShort returns the short date representation of 't' for 'ht'
func (ht *ht) FmtDateShort(t time.Time) string {
...
// FmtDateMedium returns the medium date representation of 't' for 'ht'
func (ht *ht) FmtDateMedium(t time.Time) string {

	b := make([]byte, 0, 32)

	b = strconv.AppendInt(b, int64(t.Day()), 10)
	b = append(b, []byte{0x20}...)
	b = append(b, ht.monthsAbbreviated[t.Month()]...)
	b = append(b, []byte{0x20}...)
...
// FmtDateLong returns the long date representation of 't' for 'ht'
func (ht *ht) FmtDateLong(t time.Time) string {

	b := make([]byte, 0, 32)

	b = strconv.AppendInt(b, int64(t.Day()), 10)
	b = append(b, []byte{0x20}...)
	b = append(b, ht.monthsWide[t.Month()]...)
	b = append(b, []byte{0x20}...)
...
// FmtDateFull returns the full date representation of 't' for 'ht'
func (ht *ht) FmtDateFull(t time.Time) string {

	b := make([]byte, 0, 32)

	b = append(b, ht.daysWide[t.Weekday()]...)
	b = append(b, []byte{0x20}...)
	b = strconv.AppendInt(b, int64(t.Day()), 10)
	b = append(b, []byte{0x20}...)
	b = append(b, ht.monthsWide[t.Month()]...)
...
// FmtTimeShort returns the short time representation of 't' for 'ht'
func (ht *ht) FmtTimeShort(t time.Time) string {
...
// FmtTimeMedium returns the medium time representation of 't' for 'ht'
func (ht *ht) FmtTimeMedium(t time.Time) string {
...
// FmtTimeLong returns the long time representation of 't' for 'ht'
func (ht *ht) FmtTimeLong(t time.Time) string {
...
// FmtTimeFull returns the full time representation of 't' for 'ht'
func (ht *ht) FmtTimeFull(t time.Time) string {
