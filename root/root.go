...
func (root *root) FmtCurrency(num float64, v uint64, currency currency.Type) string {
	...
	for j := len(root.currencyPositivePrefix) - 1; j >= 0; j-- {
		b = append(b, root.currencyPositivePrefix[j])
	}
...
func (root *root) FmtAccounting(num float64, v uint64, currency currency.Type) string {
	...
	if num < 0 {
		...
		for j := len(root.currencyNegativePrefix) - 1; j >= 0; j-- {
			b = append(b, root.currencyNegativePrefix[j])
		}
